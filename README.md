...
1. **CSV mode** (tracker interface): A `statutory_rates.csv.gz` file provides dense per-authority rates at HTS10×country level. Generated by `tariff-rate-tracker`'s `generate_etrs_config.R`. Optional YAML files overlay counterfactual modifications.
...
config/
  baseline/                    # Shared baseline config
    statutory_rates.csv.gz     # CSV mode: dense rates from tracker (optional)
...
When `statutory_rates.csv.gz` is present in a config directory, ETRs reads it as the baseline and applies adjustments (USMCA, metal content, MFN exemption shares, stacking). Key columns:
...
YAML files in the same directory act as counterfactual overlays: rates overwrite matching cells, new products/authorities are added, explicit zeros suppress baseline rates.

### MFN Rates

In YAML-only mode, each `other_params.yaml` must include a pointer to the MFN rates file:

```yaml
mfn_rates: 'resources/mfn_rates_2025.csv'
```

In CSV mode, MFN rates come from the `mfn_rate` column in `statutory_rates.csv.gz` (tracker is source of truth). The `mfn_rates` key is still accepted as a fallback.
