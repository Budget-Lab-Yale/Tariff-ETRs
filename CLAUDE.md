...
**Two config modes** are supported (auto-detected):

1. **CSV mode** (tracker interface): `statutory_rates.csv.gz` provides dense per-authority rates at HS10×country level. Optional YAML files overlay counterfactual modifications.
2. **YAML-only mode** (hand-authored): Individual YAML files per authority define rates via headline + product + product×country hierarchy.

```
config/
  baseline/                    # Shared baseline config (required)
    statutory_rates.csv.gz     # CSV mode: dense rates from tracker
    other_params.yaml          # Required in both modes
    s232.yaml                  # YAML-only mode, or overlay in CSV mode
  ...
  {scenario}/                  # Counterfactual configs only
    2026-01-01/                # Counterfactual date (time-varying)
      statutory_rates.csv.gz   # Each date subfolder is self-contained
      other_params.yaml
      s232_pharma.yaml         # Optional YAML overlay (counterfactual)
      ...
```
...
5. **s301.yaml** (optional) - Section 301 product-specific tariffs:
   ```yaml
   headline_rates:
     default: 0.0            # Default rate for all countries
     china: 0.25             # China-specific rate
     # ... other countries/mnemonics

   product_rates:            # Optional: HTS-specific overrides
     '8541': 0.50            # Semiconductors
   ...
   ```
   Uses the same hierarchical format and `load_ieepa_rates_yaml()` loader as IEEPA configs.
   When absent, Section 301 rates default to zero.
...
- `load_scenario_config()`: Loads config from a single directory. Auto-detects CSV vs YAML-only mode. In CSV mode: loads `statutory_rates.csv.gz` via `load_statutory_csv()`, then applies any YAML overlay files. In YAML-only mode: loads individual authority YAMLs. Returns a named list with rate tibbles, other_params, MFN rates, USMCA shares.
...
- `merge_overlay()`: Merges YAML overlay onto CSV baseline config. YAML rates overwrite matching `(hs10, cty_code)` pairs via anti_join + bind_rows. Explicit zeros suppress positive CSV rates.
...
- `calc_weighted_etr()`: Joins tabular config data, applies USMCA exemptions/auto rebates, metal content adjustment, MFN exemption shares, generalized target_total floor logic, and stacking rules. Handles NULL tariff inputs (missing config = zero rates). Supports product-level USMCA shares via `usmca_product_shares` parameter. MFN can come from two sources: (1) `mfn_rates_by_product_country` at hs10×country level from CSV (tracker source of truth, no exemption shares), or (2) `mfn_rates` at hs8 level from YAML (with optional exemption shares). Both paths apply USMCA share reduction to MFN for CA/MX.
